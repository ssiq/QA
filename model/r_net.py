...
import numpy as np
from common import tf_util, util, rnn_cell
...
class RNet(object):
    ...
    def passage_bi_rnn_op(self):
        return tf_util.bi_rnn(self._multi_rnn_cell, self.passage_embedding_op,
                              self.passage_length)[0]

    @tf_util.define_scope("question_bi_rnn_op")
    def question_bi_rnn_op(self):
        return tf_util.bi_rnn(self._multi_rnn_cell,
    ...
    def question_aware_passage_op(self):
        return tf_util.bi_rnn(
            lambda: rnn_cell.GatedAttentionWrapper(self._multi_rnn_cell(), self.question_bi_rnn_op, self.question_length,
                                                   self.hidden_state_size, ), self.passage_bi_rnn_op, self.passage_length)[0]
    ...
    def question_vector_op(self):
        Vr = tf.get_variable("question_Vr",
                             ...
                             dtype=tf.float32)
        return tf_util.soft_attention_reduce_sum(self.question_bi_rnn_op,
                                                 Vr,
                                                 self.hidden_state_size,
                                                 self.question_length)
    ...
    def answer_logit_op(self):
        with tf.variable_scope("soft_attention"):
            a0 = tf_util.soft_attention_logit(self.hidden_state_size,
        ...
        with tf.variable_scope("soft_attention", reuse=True):
            a1 = tf_util.soft_attention_logit(self.hidden_state_size,
                                              h1,
                                              self.self_matching_passage_op,
                                              self.passage_length)
        return a0, a1
